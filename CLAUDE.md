...
|---|---|---|
| **Bot** | 3.0.61 | `package.json` |
| **Agent** (`wolfpack-logsync`) | 3.0.94 | `packages/wolfpack-logsync/package.json` |
| **Web** (`wolfpack.quest`) | 1.0.45 | `web/package.json` |
| **Mimic** (Electron desktop) | 1.0.67 (stable, main; beta channel idle until next 1.0.68-beta.x) | `apps/mimic/package.json` |
...
- **Missing-spells advisor (`/me` tab).** Per character, compute the SET of spells they should have at their level + class vs the SET they've been observed casting (or that show in their spell-book inventory snapshot if/when we capture that). Surface the GAP grouped by acquisition zone: which NPC vendor sells each missing spell, and which zone has the densest concentration of "spells you need" so the user can plan a single trip. Data sources: `eqemu_spells` (we have it), the NPC merchant catalog (need to sync `eqemu_merchantlist` + `eqemu_npc_types` ↔ `merchant_id` join), zone catalog (have it). UI: a sortable "spells missing → vendor → zone" table on `/me` per character. Also opens a "coordinate with guild" surface where multiple characters needing the same vendor can sync up.
- **Faction + long-standing quest tracking.** Mine faction-change log lines (`Your faction standing with X has been adjusted by Y`) so the agent can chart each character's faction journey over time. For named quests, ingest the well-known quest scripts (Quarm publishes these), tag each observed turn-in line (`X gives you Y`) against quest steps, and surface "where you are in <quest>" on `/me`. Long-tail UX: a checklist per quest with completed/pending steps and "what to do next" hints. Storage: a `character_faction_log` table (character, faction_name, delta, ts) and a `quest_state` table keyed on (character, quest_id) with step JSONB.
- **Tradeskill advisor.** Track per-character tradeskill level via skillup log lines (`You have become better at Brewing! (X)`). Once we know current level, cross-reference against `eqemu_tradeskill_recipe` (need to sync — same weekly workflow as the rest of the EQEmu mirror) to compute: (a) recipes near current level for skillup, (b) recipes the character can MAKE right now given their inventory snapshot (need an inventory cross-reference table — start with `character_inventory` from log captures), (c) cheapest paths via `eqemu_merchantlist` joining recipe components to vendor-buyable items. Sort by: completion-now vs skillup-soon vs cheapest-per-skillpoint. UI: a `/me/tradeskills` tab with per-skill cards (Brewing, Smithing, Tailoring, etc.) showing current level, next 5 recipes by skillup proximity, total cost, in-bag vs need-to-buy breakdown.
