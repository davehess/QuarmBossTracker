    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        },
        {
          name: '📊 Parse Tracking',
          value: [
            '`/parse <data> [type]` — Submit an EQLogParser "Send to EQ" paste after a kill',
            '   **type** — `instance` *(default, starts timer)* · `open_world` · `pvp` *(stats only)*',
            '   ✅ **Guild instance kills only** trigger respawn timers. Confirm you\'re in an instance:',
            '      your chat shows **"You have incurred a lockout on the instance of…"** after the kill.',
            '      Not sure? Type `#showlootlockouts` (or `/sll`) in-game to list your current lockouts.',
            '   Multiple people can `/parse` the same kill — submissions are merged automatically.',
            '',
            '`/parseboss <boss> <data>` — Submit a parse with explicit boss selection',
            '`/parsestats <boss>` — DPS scoreboard and raidwide metrics across all kills',
            '`/parseaoe <data>` — Submit an AoE parse (merges within a 5-minute window)',
