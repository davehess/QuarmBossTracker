...
**The ask (the guild lead):** *"the dashboard having a sidebar for quick navigation is
...
- **The crash review card stays on Info.** the guild lead placed it there the same day
...
**The correction (guild lead, 2026-08-13):** asked to break out which raiders can only
make one or two nights and who joins late, the first pass answered entirely from
`opendkp_ticks` — who turned up. The guild lead pointed out that the attendance piece is
...
- **The late question improves into "who warns you".** a member declares `late` 18
  times against 14 actual late arrivals — the most-late raider is also the most
  reliable signaller. a member declares 0 across 55 firm sign-ins and is late 47%
  of nights; same shape for a member (0 / 40%) and a member (0 / 37%). The
...
which is how a member got flagged "light on Sunday" when he had actually attended
...
**The finding (a member, 2026-08-13).** Zeal data never reached Mimic; the
...
|---|---|---|
| never connects, `ENOENT` | Mimic installed *inside* the EQ folder; its DLLs shadow Zeal's DX hook (a member, 2026-06-12) | reinstall Mimic elsewhere |
| connects, then instantly closes, **no error code** | EQ elevated, Mimic not — pipe ACL denies the lower-integrity process (a member, 2026-07-05) | match elevation |
| `(EPERM)` on connect | **XP compatibility mode on eqgame.exe** (a member, 2026-08-13) | untick compatibility mode |
...
integrity level or its user, which is exactly why I told the guild lead it *couldn't* be
...
**Follow-up (the guild lead): can the pipe be exposed WITH compat mode on?** It matters —
...
**The report (guild lead, 2026-08-13).** Guild lines were reaching Discord twice —
...
**⚠ The cause written into our own code was wrong, and the guild lead's screenshot
disproved it.** `index.js` claimed EQ shows the speaker their own line as typed
and broadcasts an auto-capitalised copy to bystanders. The guild lead is a **bystander**
...
rewritten copies came from **a member** (×3) and **a member** (×1); verbatim copies
from a member's owner, the guild lead, a member and a member/a member. **Not our code** —
a member, a member, a member and a member's owner were all on agent **3.5.58**, and
that build sits on both sides of the split. Not compat mode either: a member
...
a member search his own `eqlog_*_pq.proj.txt` for one of those lines.
...
**How it surfaced.** Guild lead: *"Gear isn't showing icons… not gear, inventory."*
...
**Disabled rather than left (Guild lead: "before raid").** Wrong art is worse than no
