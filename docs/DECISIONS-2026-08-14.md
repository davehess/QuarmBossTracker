...
| Item | State |
|---|---|
| **⚠ P1 — the `bosses_local` gate silently drops first-time content (2026-08-16, task #47)** | The encounter handler refuses to persist any boss without a `bosses_local` row and still 200-acks the agent — so the night's SLEEPER'S TOMB pivot lost ~75 min of trash AND **The Final Arbiter first kill** (10 uploaders) while chat/buffs flowed normally. Timer + Discord card survived (they read `data/bosses.json`). Patched live: 7 ST mobs inserted into bosses_local; **recovery in flight** — backfill requests pending for Chadivarius/Bardtholemu/Dafeet/Lowang + Hitya running his own; verify the Arbiter + trash cards materialize as they land. Structural fix = task #47 (fall back to eqemu name resolution / NULL npc_id, never drop) + the founding sentinel invariant: "combat uploads arriving, zero encounters persisted." Full write-up: RAID-WATCH-2026-08-16.md. Also: the stage branch `claude/sharp-lamport-dC0TW-stage-web-1-1-62` is merged but UNDELETABLE from cloud (403 on ref deletes) — delete from a local session or the GitHub UI |
