...
A long live-iteration session with the guild lead testing the overlay-size feature on
...
## Tray ↔ dashboard parity is now a RULE (the guild lead)
...
layout saves existed since v1.2 and the guild lead met them tonight. The dashboard
...
row in BUILT-IN OVERLAYS (Guild lead: "Dock isn't available from the built in
...
## Release name: smoOOTH SCAlers (the guild lead) — GRADUATED to stable same day

The guild lead named the line and its rendering: *"Smoooth Scalers where it starts
...
## Overlay size — the shape that survived three rounds (the guild lead, live)
...
3. **Sliders apply on release and the overlay GLIDES (~180ms).** "Smooth
   slider" checkbox = the glide itself, **default ON** (Guild lead: "being off to
   glide doesn't make sense"); off = snap. The earlier live-follow mode is
   gone — it's what made the checkbox read backwards.
4. **The dock does NOT follow the global scale by default** (Guild lead: "don't
...
5. **Setup chrome counter-zooms** (Guild lead: "the actual slider sizing shouldn't
...
The guild lead's machine showed every overlay as a colored outline + ✥🔒 buttons while
a member's looked normal: `cfg.overlaysLocked=false` PERSISTS, and unlocked
...
## Melody AE badge counts one pulse; damage rides the chip (a member via the guild lead)
...
## Seru Minis are a GROUP EVENT, not board bosses (the guild lead, bot 3.1.56)
...
(From a member's "Seru Mini's" thread — the nudge card had mis-detected Lord
Inquisitor Seru.)

**The roster (the guild lead's correction, same hour): "they are these four"** — the
...
| **Beta line needs a field pass on today's batch** | beta.13-ish carries: size sliders ×3 surfaces + glide + counter-zoomed setup bars, dock scale opt-in, melody kite chips, layouts card, hide-all heal. The guild lead + a member are live-testing; watch for reports before graduating anything. a member's 5K at 200% still needs a real-hardware hover-target check |
| **P1 recovery tail (bot 3.1.52)** | 4 backfill requests (a member/a member/a member/a member) recover **The Final Arbiter** — all still `pending` (none has relaunched Mimic). The guild lead's own re-run heals Progenitor + Master of the Guard. Verify cards as they land. Stage branch `claude/sharp-lamport-dC0TW-stage-web-1-1-62` still needs deletion from a local session (cloud 403s ref deletes) |
| **3-tick short raids until PoP (guild lead, 2026-08-16)** | Alt raids and Seru+misc nights run 3 ticks / 2 hours until PoP. Tick math unaffected; deploy freeze deliberately full-length. Tonight (Wed) is a raid night — freeze 19:30–00:30 ET |
...
| **Guild membership in front of personal tooling?** | the guild lead's call on shape: guest role vs personal tier vs Mimic-only (task #40) |
| **Task #27 — the 8 muted trash triggers** | Gate is "the fleet is on the fix". Restore on the guild lead's word — raid-noise call |
...
| **Caustic Mist on Zlandicar's shared Putrefy line** | Fires on the shared text; no text-level fix exists — scoping is the guild lead's call |
