...
- **How it works.** The legacy client does not store other entities' targets,
  ...
  position (clickable; LMB-drag code present). Settings under `[AssistBar]`.
...
| Zeal: put Target of Target on the pipe | open — 1.4.7 computes it client-side (`assist_target.cpp`) but emits nothing; our Ext Target "→ tank" arrow is still log-inferred | follow-up PR to CoastalRedwood/Zeal: getter on `AssistTarget` + `player_data["target_of_target_id"]` in `named_pipe.cpp` (one-liner shape of #229); then Mimic reads it and the relay makes it raid-wide |
