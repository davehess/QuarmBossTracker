...
|---|---|---|
| Mimic mini mode — every overlay in a less-tall version, right-click ▭ toggle, per-overlay 📌 lock, Minimize-all hotkey (Ctrl+Shift+M) | **renditions presented 2026-09-11, awaiting Hitya's picks** — private artifact "Mimic Mini Mode" (Claude Code artifacts gallery): A/B per overlay with four-number costs; Buff queue gets category counters instead of a mini; Copy button shrinks to tab size, copied line carries `\| local` / `\| merged` (parser tolerance must be checked first); Target-info resists show current-after-debuffs over full | build nothing until picked; then ground rules first (switch + hotkey + lock + bar styles), template-only overlays next, bot payload (resist mods + pet haste) after, CH timeline last |
