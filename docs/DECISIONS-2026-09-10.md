...
- The "surprising connections" are `execute()` → OpenDKP helpers through
  ...
  `text()` — name collision).
Two calls for Hitya: (1) `graphify claude install` writes a skill + hooks
into `.claude/` that steer sessions to query the graph before reading files —
same doc-root hazard as `/impeccable init`, so not run; (2) where outputs
live if we keep them (a `docs/graphify/` report + html, regenerated by a
script, vs. nothing committed).
...
| Graphify of the codebase | **run 2026-09-13**, private artifact + report delivered; nothing committed | Hitya: install the Claude skill/hooks (`graphify claude install`) or not; keep outputs in `docs/graphify/` via a regen script, or leave uncommitted |
