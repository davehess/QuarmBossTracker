...
| Zeal: put Target of Target on the pipe | **patch drafted 2026-09-12** — `docs/zeal-tot-pipe.patch` (applies cleanly to v1.4.7; NOT compiled here) + PR text in `docs/zeal-tot-pipe-request.md`: `ResolveCandidate`/`LookupLiveEntity`/`IsActive` on `AssistTarget`, two omitted-when-absent `player` keys `target_of_target` + `target_hit_by` `{id,name,authoritative}`, README section | Hitya builds it locally, opens the PR under their account; then Mimic reads the keys and the relay makes it raid-wide |
