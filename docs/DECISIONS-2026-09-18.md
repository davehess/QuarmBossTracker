...
| **Share it in the main PQ Discord's leader channel** — `docs/marketing/discord-post.md` | **written 2026-09-18** (the guild lead: *"my guild leadership providing the writeup and screenshots to their leader-only discord channel on the main PQ discord"*). A short peer post, NOT the brochure — a community channel is not an advertising surface. Includes the three pre-flight checks, a screenshot safety table, and canned answers for the likely replies | leadership reads the channel's self-promotion rules first; **lead with the `/about` overlay montage**, which is now safe (below) |
| ⚠ **Public pages named the real raid in MOCK DATA** — the 2026-09-16 open item | **`/about` CLOSED 2026-09-18.** `web/components/about/OverlayDemo.tsx` rendered **ten real raiders across 53 occurrences** and is the page you would screenshot to show other guilds. Swapped to the sanitization pass's invented names, with a banner telling the next person not to "fix" them back | ⚠ **`/mimic/mini` is still open** (`mocks.tsx`, `miniReview.ts`) — its vote has closed so it can be swapped the same way. **Until then, do not screenshot that page** |
| **Runtime branding from Discord** — `DESIGN-guild-kit.md` §7b | **designed 2026-09-18** (the guild lead's idea). Verified: the tray icon and the running window icon ARE runtime-settable (`tray.setImage` exists and is called **zero** times today); the exe/installer icon and `appId` are compiled in, as he guessed. The guild's own Discord icon is the natural source — the bot has the client and the guild id, the link flow just never asks for it | small enough to land early: two fields on the identity response, then download-and-cache in Mimic like `ghDownload.js` does. **Makes the generic build stop being a downgrade** and removes the worst item from the de-branding list |
