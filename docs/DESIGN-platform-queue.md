...
| 2.2 | ✅ DONE (core 2026-07-18 bot 3.0.208 + agent 3.3.85; **tail 2026-07-18 as [#106]** bot 3.0.210 + agent 3.3.87 beta) [#73] Admission control: per-token budgets + 429/Retry-After, Supabase circuit breaker + request timeout, bot cache for target-buffs, **the six GET loops consolidated into one multiplexed `/api/agent/poll` (fallback-safe on 404)**, **encounter-burst jitter** (hash(uploader)%15s). See STATUS.md #106. | The durable queue already honors backoff — the bot just never asks. | M/L |
