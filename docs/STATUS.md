...
- **⏳ Zeal: icon tag shapes, numbered badges 1–12 and a wolf (branch `tag-shapes` on the guild lead's fork; first version rendered in game, 2026-09-25).** Keys `K X A D F T L` = skull, X, sword, diamond, flame, star, wolf; `^1^`–`^12^` = white numbered badges, readable from both sides. Built from convex parts with proud accent details; previewed off-client from the real geometry code. `docs/upstream/zeal-tag-shapes/`, `DECISIONS-2026-09-21.md` §27, §30.
