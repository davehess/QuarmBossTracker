...
- **⏳ Zeal: a symbol for each guild (preview only, the guild lead to pick, 2026-09-25).** 30 guilds (the 29 listed plus Wolf Pack as the `^WP^` wolf) prototyped two ways: a hand-built pictogram each, or a monogram banner each; proposed key `^#<code>^` with the banner as fallback. No C++ until picked. `docs/upstream/zeal-guild-emblems/PROPOSAL.md`, `DECISIONS-2026-09-21.md` §32.
