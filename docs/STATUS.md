...
- **⏳ Zeal: icon tag shapes, numbered badges, lettered paws and a traced wolf (branch `tag-shapes` on the guild lead's fork; first version rendered in game, 2026-09-25).** Keys `K X A D F T L M U N H` = skull, X, sword, diamond, flame, star, wolf, moon (mez), lasso (pull), lute (bard), shield (tank); `^1^`–`^12^` white numbered badges; `^P0^`–`^PZ^` the paw with a charmer's initial. The wolf is traced from the landing-page art (five variants, the guild lead to pick). Previewed off-client from the real geometry code. `docs/upstream/zeal-tag-shapes/`, `DECISIONS-2026-09-21.md` §27, §30, §31.
