...
- **⏳ Zeal: a Bandolier chat filter (branch on the guild lead's fork, builds clean locally, in-game test + PR still to do, 2026-09-25).** A Quarm Discord request: bandolier status lines flood the "Other" chat filter. Zeal prints them with the default colour; the change gives them their own Zeal → Bandolier filter (failures stay put). `docs/zeal-bandolier-filter-request.md`, `docs/zeal-bandolier-filter.patch`, `DECISIONS-2026-09-21.md` §26.
