...
- **⏳ Zeal: tags survive a crash, relog or character switch; tags from other zones ignored (branch `tag-persistence` on the guild lead's fork, not built yet, 2026-09-25).** Tags were memory-only and matched by spawn id alone. Now saved per character by zone + spawn id + name and restored; received tags must match the name. `docs/upstream/zeal-tag-persistence/`, `DECISIONS-2026-09-21.md` §28.
