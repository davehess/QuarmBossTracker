...
| **Off-tank + off-heal surfacing** | bot v3.0.135 (grace-period persistence of previously-targeted mobs + `incoming_mob` groundwork), agent v3.1.97/mimic v1.4.1 (Extended Target off-tank rows behind a toggle), agent v3.1.98/mimic v1.4.2 (CH Chain "off-heal candidates" list — raiders taking repeated single-target damage outside the MT/Rampage) |
| **/admin/links — family confirm** | web v1.0.170 — a one-click "confirm these are all one family" bulk-link on the character-linking page, plus fixing the suggested-main default from "alphabetically-first alt" to the member's actual main (Discord identity → guild rank → self-pin). Writes durable `main_name_override` (survives the OpenDKP sync) |
