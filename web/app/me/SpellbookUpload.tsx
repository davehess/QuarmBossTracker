...
// Per-character spellbook upload. EQ `/outputfile spellbook` writes
//   Index <tab> SpellId <tab> Level <tab> Name
// We store the SpellId so the downstream "who needs this spell we have"
// admin view (and the spell-share matcher on /me) joins eqemu_spells exactly.

import { useRef, useState, useTransition } from 'react';
import { uploadSpellbook } from './spellbook-actions';

export default function SpellbookUpload({ character }: { character: string }) {
  ...
  function submitText(text: string) {
    ...
    startTransition(async () => {
      const res = await uploadSpellbook(character, text);
      if (res.ok) setStatus({ kind: 'done', msg: `Parsed ${res.count} spells.` });
  ...
  return (
    <div className="text-xs">
      <button
        ...
        title="Upload your spellbook so officers can see which spells you still need"
      >
        📖 {open ? 'Close' : 'Upload spellbook'}
      </button>
      {open && (
  ...
  return (
    <div className="mt-1 space-y-1">
      <textarea
        ...
        placeholder="Paste the tab-separated spellbook output here…"
