...
// watching the repo. All stats below are measured from the repo/live system,
// not aspirational — update them when they drift.
...
// (site accent tokens, borders/icons — never the text).
const BRANCHES = [
  {
    id: 'mimic',
    icon: '🖥',
...
const TINT: Record<string, { border: string; text: string; glow: string }> = {
  ...
  return (
    ...
    >
      ...
      {BRANCHES.map((b) => {
        const p = MAP_POS[b.id];
        return (
          <a key={`n-${b.id}`} href={`#${b.id}`} aria-label={`${b.title} — jump to details`}>
