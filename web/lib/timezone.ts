// Shared TZ formatters. Default zone is America/New_York because most of the
// guild plays from EST/EDT; an explicit picker (see TimezonePicker) lets a
// member override via cookie. The cookie is read server-side via next/headers
// so server-rendered pages get the same zone the client picker shows.
//
// Always pass the zone through to toLocaleString — Vercel's container runs in
// UTC, so unqualified toLocaleString silently rendered every absolute time in
// UTC, not the user's wall clock. That's the bug we're fixing here.
